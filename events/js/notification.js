...
document
  ...
  .addEventListener("click", function () {
    ...
    if (!time || !message) {
      info.textContent = "enter time and message";
...
document
  ...
  .addEventListener("click", function () {
    if (localStorage.length && confirm("clean list of notification")) {
      localStorage.clear();
      update();
  ...
  for (let key of Object.keys(localStorage)) {
    ...
    if (
      ...
      if (!document.querySelector(".audioAlert")) {
        document
          .querySelector("body")
          .insertAdjacentHTML("afterbegin", `<audio loop class="audioAlert">
          <source src ="../source/alert.mp3" type="audio/mpeg"></source>
          </audio>`);
